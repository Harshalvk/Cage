...
func (s *Store) Save(ctx context.Context, sb *Sandbox) error {
	_, err := s.pool.Exec(ctx,
		`INSERT INTO sandboxes (id, container_id, status, created_at)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT (id) DO UPDATE SET status = $3`,
		sb.ID, sb.ContainerID, sb.Status, sb.CreatedAt,
  )
	if err != nil {
...
func (s *Store) Get(ctx context.Context, id string) (*Sandbox, error) {
	...
	err := s.pool.QueryRow(ctx,
		  `SELECT id, container_id, status, created_at FROM sandboxes WHERE id = $1`,
		  id,
	).Scan(&sb.ID, &sb.ContainerID, &sb.Status, &sb.CreatedAt)
...
func (s *Store) List(ctx context.Context) ([]*Sandbox, error) {
	rows, err := s.pool.Query(ctx,
	    `SELECT id, container_id, status, created_at FROM sandboxes ORDER BY created_at DESC`,
	)
	if err != nil {
	...
	for rows.Next() {
		var sb Sandbox
		if err := rows.Scan(&sb.ID, &sb.ContainerID, &sb.Status, &sb.CreatedAt); err != nil {
